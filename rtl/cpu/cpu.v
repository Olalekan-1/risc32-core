module cpu(input clk, reset, pc_en,
    ...
    wire reg_write_en, pc_src, reg_src_a, reg_src_b, alu_src;
    wire [3:0] alu_flags;
    wire [1:0] imm_src;
    ...
    datapath datapath_i(.clk(clk), .reset(reset),.reg_write_en(reg_write_en), .mem_write_en(mem_write_en), 
                        ...
                        .alu_result(alu_result), .pc(pc), .read_data(read_data), .instr(instr), .alu_flags(alu_flags),
                        .pc_en(pc_en)
                        );
